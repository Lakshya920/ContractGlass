...
if use_demo:
    ...
    # The text_area below is bound to key="preview_area", so once it has rendered once,
    # Streamlit reads its displayed value from st.session_state["preview_area"] on every
    # rerun rather than from the `value=` argument. We must update that key directly too,
    # or the preview will keep showing whatever scenario was loaded first.
    st.session_state.preview_area = MOCK_CONTRACTS[selected_demo]
    st.session_state.analysis_result = None
    st.session_state.email_draft = None
    st.session_state.previous_file_name = None
    ...
    if extracted_text:
        ...
        st.session_state.preview_area = extracted_text  # keep in sync, same reason as above
        st.session_state.analysis_result = None
        st.session_state.email_draft = None
